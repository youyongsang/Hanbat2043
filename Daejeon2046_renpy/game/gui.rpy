...
## (한 블록에 여러 문장을 \n으로 묶어서 쓰는 방식이라 넉넉하게 잡음)
define gui.nvl_list_length = 20

## The height of an NVL-mode entry. Set this to None to have the entries
## dynamically adjust height.
## 대전2046은 블록 하나에 여러 줄이 들어가서 고정 높이로는 겹침이 생기므로 None으로.
...
## 대전2046은 이름표를 아예 안 보여주는 단일 칼럼 구성이라(원본 템플릿의
## "이름 칸 + 대사 칸" 2단 레이아웃 값을 그대로 두면 텍스트가 오른쪽으로 쏠림),
## 전체 폭(1080)을 다 쓰도록 직접 재정의함.
...
## The position, width, and alignment of the dialogue text.
define gui.nvl_text_xpos = gui.scale(48)
define gui.nvl_text_ypos = gui.scale(8)
define gui.nvl_text_width = gui.scale(984)
define gui.nvl_text_xalign = 0.0

## The position, width, and alignment of nvl_thought text (the text said by
## the nvl_narrator character.)
define gui.nvl_thought_xpos = gui.scale(48)
define gui.nvl_thought_ypos = gui.scale(0)
define gui.nvl_thought_width = gui.scale(984)
define gui.nvl_thought_xalign = 0.0

## The position of nvl menu_buttons.
define gui.nvl_button_xpos = gui.scale(48)
define gui.nvl_button_xalign = 0.0
define gui.nvl_button_width = gui.scale(984)
...
init python:
    ...
    def small():
        ...
        ## NVL-mode. (대전2046: 이름표 없는 단일 칼럼, 전체 폭 사용 — 위쪽 기본값과 동일하게 맞춤)
        gui.nvl_height = None

        gui.nvl_name_width = 0
        gui.nvl_name_xpos = 0

        gui.nvl_text_width = gui.scale(984)
        gui.nvl_text_xpos = gui.scale(48)
        gui.nvl_text_ypos = gui.scale(5)

        gui.nvl_thought_width = gui.scale(984)
        gui.nvl_thought_xpos = gui.scale(48)

        gui.nvl_button_width = gui.scale(984)
        gui.nvl_button_xpos = gui.scale(48)
